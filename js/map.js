...
(function () {
  ...
  window.util.mapPin.addEventListener('mousedown', function (evt) {
    ...
    var onMouseMove = function (moveEvt) {
      ...
      var INACTIVE_PIN_Y = MAIN_PIN_Y - PART_ACTIVE_PIN_Y;
      var MAIN_PIN_Y_ADDRESS = mapFaded ? INACTIVE_PIN_Y : MAIN_PIN_Y;
      var MIN_PIN_Y_ADDRESS = mapFaded ? (MIN_MAP_Y - PART_ACTIVE_PIN_Y) : MIN_MAP_Y;
      var MAX_PIN_Y_ADDRESS = mapFaded ? (MAX_MAP_Y - PART_ACTIVE_PIN_Y) : MAX_MAP_Y;
      ...
      if (((newCoordsY + MAIN_PIN_Y) >= MIN_MAP_Y && (newCoordsY + MAIN_PIN_Y) <= MAX_MAP_Y) &&
        ...
        addressInput.setAttribute('value', (newCoordsX + MAIN_PIN_X) + ', ' + (newCoordsY + MAIN_PIN_Y_ADDRESS));
      }
      if (((newCoordsY + MAIN_PIN_Y) < MIN_MAP_Y) && (!bordersY.includes(mapPin.style.top)) && (!bordersX.includes(mapPin.style.left))) {
        ...
        addressInput.setAttribute('value', (newCoordsX + MAIN_PIN_X) + ', ' + MIN_PIN_Y_ADDRESS);
      }
      if (((newCoordsY + MAIN_PIN_Y) > MAX_MAP_Y) && (!bordersY.includes(mapPin.style.top)) && (!bordersX.includes(mapPin.style.left))) {
        ...
        addressInput.setAttribute('value', (newCoordsX + MAIN_PIN_X) + ', ' + MAX_PIN_Y_ADDRESS);
      }
      if ((newCoordsX < -MAIN_PIN_X) && (!bordersX.includes(mapPin.style.left)) && (!bordersY.includes(mapPin.style.top))) {
        ...
        addressInput.setAttribute('value', 0 + ', ' + (newCoordsY + MAIN_PIN_Y_ADDRESS));
      }
      if ((newCoordsX > widthMap - MAIN_PIN_X) && (!bordersX.includes(mapPin.style.left)) && (!bordersY.includes(mapPin.style.top))) {
        ...
        addressInput.setAttribute('value', widthMap + ', ' + (newCoordsY + MAIN_PIN_Y_ADDRESS));
      }
    };
